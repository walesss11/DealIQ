...
export default function LoginForm() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(data.redirectTo || returnUrl);
      router.refresh();
    } catch (err: unknown) {
  ...
  async function handleGoogleSignIn() {
    ...
    // If Google Dev Demo is enabled on localhost
    if (isGoogleDevDemo) {
      try {
        const devEmail = prompt("Enter test Google account email (e.g. alex@gmail.com):", "alex.creator@gmail.com");
        if (!devEmail) {
    ...
    <div className="min-h-screen flex flex-col justify-center py-12 px-6 lg:px-8 bg-slate-50 bg-subtle-pattern text-slate-900">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-xs sm:text-sm text-slate-600">
          Welcome back. Continue to your contract workspace.
        </p>
      </div>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-bold text-slate-900" htmlFor="password">
                  Password
                </label>
                <Link
            ...
            >
              {loading ? "Signing in..." : "Sign in to PactIQ"}
            </button>
          </form>
